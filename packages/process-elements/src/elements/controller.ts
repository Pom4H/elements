...
const ioModule = defineFragment({
  ...
  template: svg`<g><rect class="module" width="88" height="188" rx="6" data-part="module-body"/><path class="top-bevel" d="M6 1 H82 Q87 1 87 7 V24 H1 V7 Q1 1 6 1Z"/><text class="module-name" x="10" y="18" data-part="module-name">DI 8×24V</text><text class="module-sub" x="10" y="32" data-part="module-sub" data-detail="fine">DIGITAL INPUT</text><rect class="label-box" x="10" y="39" width="68" height="14" rx="2" data-detail="standard"/><text class="label-dark" x="44" y="49" text-anchor="middle" data-part="module-range" data-detail="standard">I0.0 — I0.7</text><g data-part="channel-bank">${channelIndicators()}</g><g data-part="terminal-bank">${terminalBank()}</g><path class="din-clip" d="M23 187 H65 L59 198 H29 Z"/></g>`,
});
...
`,
  attributes: {
    ...
    scanRate: attribute.number('scanRate', { attribute: 'scan-rate', defaultValue: 10, minimum: 0.1, step: 0.5, unit: 'Hz', description: 'Scan cycles per second.' }),
    load: attribute.number('load', { defaultValue: 0, minimum: 0, maximum: 100, step: 1, unit: '%', description: 'Controller load from 0 to 100.' }),
    activity: attribute.number('activity', { defaultValue: 0, description: 'Monotonic communication activity sequence.' }),
    status: attribute.enum('status', ['idle', 'normal', 'warning', 'alarm'] as const, { defaultValue: 'idle', description: 'Controller process status.' }),
    quality: attribute.enum('quality', ['good', 'stale', 'bad'] as const, { defaultValue: 'good', description: 'Controller data quality.' }),
  ...
  bindings: [
    ...
    bind.text('display', (context) => stateValue(context, 'alarm') ? 'FAULT' : stateValue(context, 'running') ? 'RUN' : 'STOP', ['running', 'alarm']),
  ...
  motions: [
    ...
    { id: 'alarm-pulse', type: 'loop', target: 'status-led', active: (context) => stateValue(context, 'alarm'), keyframes: [{ opacity: .35 }, { opacity: 1 }, { opacity: .35 }], options: { duration: 700, iterations: Infinity, easing: 'ease-in-out' }, reducedMotion: 'finish' },
  ],
  ports: [
  ...
  parts: [
    ...
    { name: 'status-led', detail: 'essential' },
